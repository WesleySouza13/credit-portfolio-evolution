...
class MonteCarlo:
    ...
    def stress_cenario(self, intensidade=2.0):
        mean, std, cov = self._base_params()
        shock_signal = np.array(
            [np.sign(self.stress_dict[c]) for c in self.cols_sim]
        )
        shock_mu = mean + intensidade * shock_signal * std
        ...
        for s in range(self.cenary):
            ...
            dfs.append(df_s)
        df_sim = pd.concat(dfs, ignore_index=True)
        print(f'CENÁRIO PESSIMISTA | {self.cenary} cenários | {intensidade}σ')
        return df_sim

    def optimism(self, optimism_dict, intensidade=1.0):
        mean, std, cov = self._base_params()
